...
/*   Updated: 2020/11/18 20:38:04 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void
	add_sprites(t_pos_fl coord, t_mlx *m)
{
	t_sprites	*tmp;
	t_sprites	*cast;
	t_list		*cursor;
...
void
	draw_text_sprites(t_coord_sprite sp, t_pos_fl t, t_pos_fl h_w, t_mlx *m)
{
	t_pos	coord_text;
	char	*color;
...
void
	calc_sp(t_sprites *c, t_coord_sprite *h_t, t_coord_sprite *s, t_mlx *m)
{
	h_t->column.x = (double)(m->p.r[1]) / c->distance;
...
void
	itersp(t_coord_sprite sp, t_mlx *m, t_sprites *cast, t_coord_sprite hw_tmp)
{
	while (sp.lines.x < sp.lines.y && sp.column.x >= 0
		&& sp.column.x < m->p.r[0]
			&& cast->distance < m->distance.distance[(int)sp.column.x])
	{
